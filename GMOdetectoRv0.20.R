...
crop_and_plot <- function(mode="whole_plate", grid_item, image_spectrum_table = image_spectrum_table_colored, image_type="hyperspectral",
  ...
  if(mode=="whole_plate" | mode=="both"){
    ...
    if(image_type=="crop_segment_output"){
      ...
      image_spectrum_table_all_cropped <- image_spectrum_table[which(image_spectrum_table$x < crop_bottom & image_spectrum_table$x > crop_top & image_spectrum_table$y>crop_left & image_spectrum_table$y<crop_right),]
