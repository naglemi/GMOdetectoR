...
reduce_image <- function(image_in = image_spectrum, FP="DsRed"){
  ...
  colnames(image_spectrum_loaded) <- format(seq(from=399.8645,to=799.8529, by=((799.8529-399.8645)/317)), digits=7)
  image_table <- as.data.table(cbind(image_in$y, image_in$x,
                                     image_spectrum_loaded[,'672.4118'],
...
server <- function(input, output, session) {
   ...
   output$image <- renderPlot({
     ...
     crop_and_plot(mode=what_to_plot(), image_spectrum_table = image_spectrum_table_colored(), grid_item = input$grid_position)
