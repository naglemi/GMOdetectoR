...
crop_and_plot <- function(mode="whole_plate", grid_item, image_spectrum_table = image_spectrum_table_colored, image_type="hyperspectral",
                          first_pass_FP_threshold, first_pass_Chl_threshold){
    
    if(image_type=="CLS"){
      image_spectrum_table <- CLS_workflow(img_in_to_CLS = img_in_backend,
                                           pass_FP_threshold_from_input = first_pass_FP_threshold,
                                           pass_Chl_threshold_from_input = first_pass_Chl_threshold)
      print("We have a CLS table!")
    }
...
choose_pixels <- function(FP_lambda='582.6952', Chl_lambda='508.763', full_spectrum=this_full_spectrum, CLS_threshold_FP, CLS_threshold_Chl){
  ...
  pixels_with_significant_FP <- all_pixel_indices[which(full_spectrum[,eval(FP_lambda)] > CLS_threshold_FP)]
  pixels_with_significant_Chl <- all_pixel_indices[which(full_spectrum[,eval(Chl_lambda)] > CLS_threshold_Chl)]
  pixels_with_something <- unique(c(pixels_with_significant_FP, pixels_with_significant_Chl))
  return(pixels_with_something)
}

model_and_record_single_pixel <- function(i, Xmatrix = mm){
  model_out <- fastLmPure(Xmatrix, img_in_backend$spc[i,])
...
finish_processing_CLS_table <- function(CLS_table, chosen_pixels_in = chosen_pixels){
  ...
  CLS_table$color <- rgb(red= CLS_table$ChlB,
                         ...
                         maxColorValue = max(CLS_table$DsRed))
}

CLS_workflow <- function(img_in_to_CLS, pass_FP_threshold_from_input, pass_Chl_threshold_from_input){
  ...
  read_plot_spectra <- function(spectra_path, plot=TRUE){
    ...
    wavelengths <- img_in_to_CLS@wavelength
  ...
  convert_NA_to_zero_in_spectrum_and_return_vector <- function(spectrum_in){
    vector <- spectrum_in$'Normalized intensity (fitted)'
  ...
  mm <<- cbind(1, DsRed_vector, ZsYellow_vector, ChlA_vector, ChlB_vector)
  
  print("Making CLS table")
  CLS_table <<- data.table(cbind(img_in_to_CLS$x, img_in_to_CLS$y, matrix(nrow=dim(img_in_to_CLS)[1], ncol=4)))
  colnames(CLS_table) <- c("x", "y", "DsRed", "ZsYellow", "ChlA", "ChlB")
  print("About to perform CLS")
  chosen_pixels <<- choose_pixels(full_spectrum=img_in_to_CLS$spc,
