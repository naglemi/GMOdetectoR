...
crop_and_plot <- function(mode="whole_plate", grid_item, image_spectrum_table = image_spectrum_table_colored){
    ## Now for cropping
    ...
    if(mode=="single_explant"){
      ...
      p <- ggplot(data=image_spectrum_table_single_cropped, aes(x=cols, y=rows, fill=color)) +
        coord_equal() + geom_tile() + scale_fill_identity() +
        theme(axis.ticks = element_blank(), axis.title = element_blank(),
              panel.background = element_blank(), axis.text = element_blank(),
              panel.border = element_blank(), panel.grid.major = element_blank(),
...
server <- function(input, output) {
   ...
   image_spectrum_table_colored <- reactive({
     image_spectrum_table_colored <- extract_plot_grid_item(image_spectrum_table = img_in_backend,
   ...
   output$whole_plate_image <- renderPlot({
     ...
     crop_and_plot(mode="whole_plate", image_spectrum_table = image_spectrum_table_colored())
   })
   
   output$single_explant_image <- renderPlot({
     crop_and_plot(mode="single_explant", grid_item = input$grid_position, image_spectrum_table = image_spectrum_table_colored())
   })
   ###################################################################################
